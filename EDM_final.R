...
save(E_smapc_out, file = 'E_smapc_out_2021flu_o3_G.rda')

# load('E_smapc_out_2021flu_o3_G.rda')
...
E_smapc =E_smapc_out %>% filter(E !=1) %>%
  group_by(dis,ST) %>%
...
load('sd_sparAsIs.rda')
sd_data=sd_sparAsIs

#################################### 1. CCM_OUT: perform CCM for real and surrogate data ###################################

##### 1) Perform CCM for real and surrogate data #####
...
save(ccm_out,file = 'ccm_out_2021flu_o3_H_alpha.rda')
...
save(theta_out, file = 'theta_out_2021flu_o3_G.rda')
...
save(df_123_fluP, file = 'mfi_out_2021flu_o3_H.rda')
  ...
  function(data,ST,plt_1,dis,tp_value){
    ...
    out <- foreach(i = 1:(num_surr+1),
                   ...
                   .inorder=FALSE)  %dopar% {
                     ...
                     out=rbind(out_1) %>% mutate(i=i,ST=ST,plt_1=plt_1,dis=dis,tp_value=tp_value)
                     out
                   }
    out
  }
...
save(mfi_surr_out,file = 'mfi_surr_out_2021flu_o3_H_alpha.rda')
...
save(C_out, file = 'C_out_2021flu_o3_H.rda')

# load('C_out_2021flu_o3_H.rda')

head(C_out %>% arrange(plt),n=100)
