...
p_function=function(x,pH,pL){
  dfg2_Q1=dfg2 %>% filter(grp=='surr') %>%
    group_by(dis,plt,tp_value,E,ST) %>%
...
fgam_o3=function(dat, Y='dis', x1='value', xlag=0,cityname){
  
  df  =  dat %>% filter(state == cityname) %>%
    mutate(loglag1 = log(Lag(fluP, 1)))%>%
    mutate(loglag2=Lag(loglag1,1))
  
  fit=gam(as.formula(paste(Y,
                           '~Lag(',x1,',',xlag,')+
                           s(Lag(temp,0),k=3+1,fx=T,bs="cr")+
                           s(Lag(temp,1),k=3+1,fx=T,bs="cr")+
                           s(Lag(temp,2),k=3+1,fx=T,bs="cr")+
                           s(Lag(ah,0),k=3+1,fx=T,bs="cr")+
                           s(Lag(ah,1),k=3+1,fx=T,bs="cr")+
                           s(Lag(ah,2),k=3+1,fx=T,bs="cr")+
                           # s(Lag(o3,0),k=3+1,fx=T,bs="cr")+
                           # s(Lag(o3,1),k=3+1,fx=T,bs="cr")+
                           # s(Lag(o3,2),k=3+1,fx=T,bs="cr")+
...
fgam_ah=function(dat, Y='dis', x1='value', xlag=0,cityname){
...
fgam_temp=function(dat, Y='dis', x1='value', xlag=0,cityname){
...
roundFunc  =  function(x){round(x, 4)}
